...
function doOver(evt) {
	var toEl = getReal(evt?evt.target : window.event.toElement, "className", "coolButton");
	var fromEl = getReal(evt?evt.target : window.event.fromElement, "className", "coolButton");
...
function doDown(evt) {
	el = getReal(evt?evt.target : window.event.srcElement, "className", "coolButton");
...
function disable(el) {
	...
	if (!cDisabled) {
		...
		el.innerHTML = '<span style="width: 100%; height: 100%; text-align: center;">' +
...
document.write(".coolButton {border: 1px solid #CCCC99; padding: 1px; text-align: center; cursor: default;}");
