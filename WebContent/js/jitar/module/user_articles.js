 ...
 * 用户分类文章列表.
 * 此文件作为分页的样版例子
 * 实现功能：一个页面允许出现多个相同的模块，需要考虑的问题：避免重复的html Element id和调用函数
 * 实现方式：添加模块的 loadPage 方法，替换ftl中指定的id为id + widget.id
 * */
(function() {

  var mod_name = 'user_articles';
  ...
  function _load_widget(/*widget*/w, page) {
    ...
    //var url = CommonUtil.getCurrentRootUrl() + 'u/' + user.name + '/module/' + mod_name + '?cid=' + cid + '&page=' + page + '&tmp=' + (new Date());
    var url = CommonUtil.getCurrentRootUrl() + 'u/' + user.name + '/py/user_category_article_list.py?cid=' + cid + '&page=' + page + '&tmp=' + (new Date()).getTime();
