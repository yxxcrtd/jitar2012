...
<html xmlns="http://www.w3.org/1999/xhtml">
 <head>
  ...
  var url = "${SiteUrl}push/admin_push_resource_list.py?cmd=push";
  ...
  {
  	postData = 'resourceId=' + oChk.value;
  ...
  function CheckConnection()
  {
    ...
    var url = '${SiteUrl}push/admin_push_resource_list.py?cmd=check&tmp=' + Math.random();
...
<h2>资源推送管理</h2>
<#if resource_list??>
<form id='oForm' method='POST' action='${SiteUrl}push/admin_push_resource_list.py'>
...
<#if r.unitId?? >
<#assign unit = Util.unitById(r.unitId)>
</#if>
<#assign pushuser = Util.userById(r.pushUserId)>
<tr>
<td><input type='checkbox' name='guid' value='${r.resourceId}' /></td>
<td>
<img src='${Util.iconImage(r.href!)}' border='0' align='absmiddle' />
<a href='${SiteUrl}showResource.py?resourceId=${r.resourceId}' target='_blank'>${r.title}</a></td>
...
<td><#if r.pushState == 2><span style='color:red'>待推送</span></#if></td>
</tr>
</#list>
</table>
...
<input type='button' class='button' value='推送资源' onclick='ProcessData()' />
