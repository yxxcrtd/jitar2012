...
<div class="mt3 border">
 <h3 class="h3Head textIn"><a href="${SiteUrl}go.action?unitName=${punit["p"].unitName!?url}">${punit["p"].unitTitle!?html}</a></h3>
   <div class="clearfix">
    <div style="background:#fff;padding:10px 0 10px 20px">
      <#list punit["clist"] as uu>
        <a style="display:block;width:220px;margin:6px;background:#dedede;text-align:center;padding:4px;float:left" href='${SiteUrl}go.action?unitName=${uu.unitName}'<#if uu.unitTitle?length &gt; 15> title="${uu.unitTitle!?html}"</#if>>${Util.getCountedWords(uu.unitTitle!?html,15)}</a>     
      </#list>
