	 ...
	 <div class='gly_right'>
	      ...
	      <div style='clear:both;height:10px;'></div>
			...
			<table style='margin:4px;' border='0'>
			  ...
			  <tr>
			  <td class='rank_left'>${ph_index + 1}&nbsp;</td>
			  <td class='rank_right'>
			  <#if UserUrlPattern??>
...
<div class='gly'>
  <div class='gly_head'>
    <div class='gly_head_right'><a href='showPhotoList.action?categoryId=${pc.categoryId}'>更多…</a></div>
    <div class='gly_head_left'>&nbsp;<img src='${ContextPath}css/index/j.gif' />&nbsp;${pc.categoryName!?html}分类图片</div>
  </div>
	      ...
	      <table border='0' width='160' align='left'>
	        <tr align='center'>
	         <td style='height:110px;vertical-align:top;'>
	          <a href='${UserUrlPattern.replace('{loginName}',p.loginName)}photo/${p.photoId}.html'><img style='visibility:hidden' onload='CommonUtil.reFixImg(this,150,100)' src='${Util.thumbNails(p.href!)}' 
	            alt='${p.title!?html}' border='0' /></a></td>
	        </tr>
	        <tr align='center'>
	         <td><a href='${UserUrlPattern.replace('{loginName}',p.loginName)}photo/${p.photoId}.html'>${p.title!?html}</a></td>
	        </tr>
	       </table>
      <#else>      
	        <table border='0' width='160' align='left'>
	        <tr align='center'>
	         <td style='height:110px;vertical-align:top;'>
	          <a href='${SiteUrl}${p.loginName}/photo/${p.photoId}.html'><img style='visibility:hidden' onload='CommonUtil.reFixImg(this,150,100)' src='${Util.thumbNails(p.href!)}' 
	            alt='${p.title!?html}' border='0' /></a></td>
	        </tr>
	        <tr align='center'>
	         <td><a href='${SiteUrl}${p.loginName}/photo/${p.photoId}.html'>${p.title!?html}</a></td>
	        </tr>
	        </table>
      </#if>
...
<div class='gly'>
  <div class='gly_head'>
    <div class='gly_head_left'>&nbsp;<img src='${ContextPath}css/index/j.gif' />&nbsp;所有图片</div>
  </div>
  <div class='gly_content'>
  
 <#if photo_list_all.size() &gt; 0 >
    <#-- 定义要显示的列数 columnCount -->
    <#assign columnCount = 6>
    <#-- 计算显示当前记录集需要的表格行数 rowCount -->
    <#if photo_list_all.size() % columnCount == 0>
    <#assign rowCount = (photo_list_all.size() / columnCount) - 1>
    <#else>
    <#assign rowCount = (photo_list_all.size() / columnCount)>
    </#if>
    ...
    <#list 0..columnCount - 1 as cell >
        ...
        <#if photo_list_all[row * columnCount + cell]??>                     
            <#assign photo = photo_list_all[row * columnCount + cell]>
            <#if UserUrlPattern??>
	            <div style='height:134px;background:#EEE;'>
	            <a href='${UserUrlPattern.replace('{loginName}',photo.loginName)}photo/${photo.photoId}.html'>
	            <img onload="CommonUtil.reFixImg(this,128,128)" src="${Util.thumbNails(photo.href!'images/default.gif')}" vspace='4' border='0' /></a>
	            </div>
	            <div>
            ...
            <#else>
	            <div style='height:134px;background:#EEE;'>
	            <a href='${SiteUrl}${photo.loginName}/photo/${photo.photoId}.html'>
	            <img onload="CommonUtil.reFixImg(this,128,128)" src="${Util.thumbNails(photo.href!'images/default.gif')}" vspace='4' border='0' /></a>
	            </div>
	            <div>
