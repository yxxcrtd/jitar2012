...
<th>真实姓名</th>
<th>
<#if !objectType??><#assign objectType = 0></#if>
<select onchange="doFilter(this)">
<option value=''>管理员类型</option>
<option value='4'<#if objectType == 4> selected='selected'</#if>>机构系统管理员</option>
<option value='5'<#if objectType == 5> selected='selected'</#if>>机构用户管理员</option>
<option value='6'<#if objectType == 6> selected='selected'</#if>>机构内容管理员</option>
</select>
</th>
<th>所管理的机构名称</th>
<th>管理员所在的机构名称</th>
...
<td><a href='${SiteUrl}go.action?unitId=${ul.objectId}' target='_blank'>${ul.objectTitle!?html}</a></td>
