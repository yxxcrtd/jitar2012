 ...
 * 
 *
 * @deprecated 被 /WEB-INF/py/show_user_resource_category.py 取代了.
 */
public class ResourceCategoryBean extends BaseShowUser {
	/** 分类服务. */
	...
	protected void handleRequst() throws IOException, ServletException {
		...
		String itemType = CommonUtil.toUserResourceCategoryItemType(user_model.getUserId());
		Category category;
		if (cid == 0) {
		...
		Page page = super.getSystemPageWithUserSkin(
				PageKey.SYSTEM_USER_RESCATE, user_model._getUserObject()); 
		if (page == null) {
		...
		String template_name = "/WEB-INF/user/default/resource_category_index.ftl";
	...
	private boolean parsePathInfo() {
		...
		if ("rescate".equals(parts[2]) == false) return false;
