...
<html xmlns="http://www.w3.org/1999/xhtml">
 <head>
  ...
  <script type="text/javascript" src="js/jitar/changeurlyear.js"></script>
 </head>
<body>
<#include 'site_head.ftl'>
<div class='r_s'>
<!-- 文章搜索表单 -->
<form  method='get' name="profile_form" action="articles.action">
<table align='center' border='0'>
<tr valign='top'>
<td>
  关键字：<input class='s_input' size="20" name='k' value="${k!?html}" />   
...
<div id='main'>
  <div class='main_left'>
    <div class='res1'>
      <div class='res1_c'>
        ...
        d = new dTree("d");
        d.add(0,-1,"<b>文章分类</b>","articles.py?type=${type!}&year=${year!}");
      <#if blog_cates??>
         <#list blog_cates.all as c>
         <#if c.parentId??>
          	d.add(${c.id},${c.parentId},"${c.name}","articles.action?type=${type}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&categoryId=${c.id}&year=${year!}");
          <#else>
          	d.add(${c.id},0,"${c.name}","articles.action?type=${type}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&categoryId=${c.id}&year=${year!}");
          </#if>
          </#list>
       </#if>
        document.write(d);
  ...
  <div class='main_right'>
    <div class='tab_outer'>
      ...
      <div class="${(type == 'rcmd')?string('cur','') }"><a href='articles.action?type=rcmd&categoryId=${categoryId!?default('')}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&year=${year!}'>编辑推荐</a></div>
      <div class="${(type == 'famous')?string('cur','') }"><a href='articles.action?type=famous&categoryId=${categoryId!?default('')}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&year=${year!}'>名师文章</a></div>
          <div class="${(type == 'new')?string('cur','')} "><a href='articles.action?type=new&categoryId=${categoryId!?default('')}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&year=${year!}'>最新发布</a></div>
          <div class="${(type == 'hot')?string('cur','') }"><a href='articles.action?type=hot&categoryId=${categoryId!?default('')}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&year=${year!}'>点击率</a></div>
          <div class="${(type == 'digg')?string('cur','') }"><a href='articles.action?type=digg&categoryId=${categoryId!?default('')}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&year=${year!}'>按顶排序</a></div>
          <div class="${(type == 'trample')?string('cur','') }"><a href='articles.action?type=trample&categoryId=${categoryId!?default('')}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&year=${year!}'>按踩排序</a></div>
          <div class="${(type == 'star')?string('cur','') }"><a href='articles.action?type=star&categoryId=${categoryId!?default('')}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&year=${year!}'>按星排序</a></div>
          <div class="${(type == 'cmt')?string('cur','') }"><a href='articles.action?type=cmt&categoryId=${categoryId!?default('')}&k=${k!?url}&gradeId=${gradeId!?default('')}&subjectId=${subjectId!?default('')}&year=${year!}'>评论最多</a></div>
          <#if backYearList??>
          <span style="float:right;padding:2px 10px 0 0">
          <select onchange="getHistoryArticle(this.value)">
