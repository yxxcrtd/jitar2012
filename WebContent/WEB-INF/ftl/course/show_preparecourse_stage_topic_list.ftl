...
<body>
    ...
    <div id='placerholder1' title='${currentStage.title!?html}的讨论' style='display:none;padding:1px;'>    
       <ul class='listul'>
		<#list topic_list as t>
		  <#assign u = Util.userById(t.userId)>
		  <li><span style='float:right'><a href='${SiteUrl}go.action?loginName=${u.loginName}'>${u.nickName}</a> ${t.createDate?string('MM-dd')}</span><a href='show_preparecourse_stage_topic_detail.py?prepareCourseTopicId=${t.prepareCourseTopicId}' target='_blank'>${t.title!?html}</a></li>
		 </#list>
        </ul>
    <#include ('pager.ftl') >
    </div>    
