 ...
 */
(function() {
  logger.log('messages module is loading...');
  var m = App._module_infos.messages; 
  // 如果已经加载过了，则不要重复加载
  if (m.loaded) return;
  ...
  function _load_widget(/*widget*/w) {
  	var url = JITAR_ROOT + 'u/' + user.name + '/module/messages?tmp=' + (new Date()).getTime();
  ...
  delete m.loading;
  m.loaded = true;
  logger.log('messages module js is loaded');
  App.onModuleLoaded('messages');
