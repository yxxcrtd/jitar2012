...
class admin_user_manager_add(BaseAdminAction, SubjectMixiner):
    ...
    def execute(self):
        ...
        if request.getMethod() == "POST":
            ...
            for userId in uid:
                # 检查是否已经是管理员
                isAdmin = accessControlService.checkUserAccessControlIsExists(userId, AccessControl.OBJECTTYPE_SYSTEMUSERADMIN, 0)
                #print "isAdmin = ", isAdmin
                if False == isAdmin:                        
                    accessControl = AccessControl()
                    accessControl.setUserId(userId)
                    accessControl.setObjectType(AccessControl.OBJECTTYPE_SYSTEMUSERADMIN)
                    accessControl.setObjectId(0)
                    accessControl.setObjectTitle(u"网站用户")
                    accessControlService.saveOrUpdateAccessControl(accessControl)
            response.sendRedirect("admin_user_manager.py")
            
        return "/WEB-INF/ftl/admin/admin_user_manager_add.ftl"
