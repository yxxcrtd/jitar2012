...
<#list article_list as article>
  ...
  <tr>
    <td><input type='checkbox' name='prepareCourseArticleId' value='${article.prepareCourseArticleId}' /></td>
    <td><a href='${SiteUrl}showArticle.action?articleId=${article.articleId}' target='_blank'>${article.title!?html}</a></td>
    <td><a href='${SiteUrl}go.action?loginName=${u.loginName}' target='_blank'>${u.nickName}</a></td>
    <td>${article.createDate?string('MM-dd hh:mm')}</td>
    <td>${article.pcsTitle}</td>
  </tr>
  </#list>
  </tbody>
</table>
...
function select_all() {
  var ids = document.getElementsByName('prepareCourseArticleId');
...
function move_article(oForm)
{
   ...
   if(confirm('你真的要将文章转移到【' + oForm.stageId.options[oForm.stageId.selectedIndex].text + '】吗?'))
    {
