...
# 协作组资源分类管理.
class group_resource_category(ActionExecutor, CategoryMixiner):
  def __init__(self):
  ...
  def dispatcher(self, cmd):
    ...
    # 计算协作组资源分类 itemType.
    self.itemType = self.toGroupResourceCategoryItemType(self.group)
  ...
  def list(self):
    ...
    return "/WEB-INF/ftl/group/group_cate_list.ftl"
  ...
  def delete(self):
    ...
    # 这里现在没有放在事务里面执行.
    self.updateGroupResourceCategoryId(self.category)
