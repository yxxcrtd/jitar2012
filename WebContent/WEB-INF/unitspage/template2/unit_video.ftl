...
<div class='tab_outer'>
    ...
    <#list 0..columnCount - 1 as cell >
        ...
        <#if video_list[row * columnCount + cell]??>                     
            ...
            <a href='${SiteUrl}manage/video.action?cmd=show&videoId=${p.videoId}'><img border=0 src="${p.flvThumbNailHref!?html}"/></a><br />
            <a href='${SiteUrl}manage/video.action?cmd=show&videoId=${p.videoId}'>${p.title!?html}</a>
        <#else>
            &nbsp;
        </#if>
        </td>
