    ...
    //alert(id);
      ...
      new Ajax.Request(url, {
        ...
        onSuccess: function(xport) { 
            ...
            var a = html.split("!!!+!!!*!!!");
            document.getElementById("_contentinputDiv").innerHTML=a[1];
            var sId = a[0].split(",");
            for(i=0;i<sId.length;i++){
                if(sId[i]!=""){
                    var editor = UE.getEditor('DHtml'+sId[i]);
                }
...
<div>
    ...
    <tr>
    <td>授课人单位：</td><td>${unitName!}</td>
    </tr>    
    <tr>
    <td>辅导老师：</td><td>${evaluationPlan.fdTeacher!}</td>
    </tr>    
    <tr>
