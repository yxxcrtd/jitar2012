...
calendar = {
	...
	makeCalendar:function(year, month, day) {
		...
		var current_year = this.today.getYear()-5;
		if(current_year < 1900) current_year += 1900;
		
		//added by 孟宪会
		var maxYear = current_year+20
		if(year < current_year) current_year = year
		if(year > maxYear) maxYear = year
		for(var i=current_year; i<=maxYear; i++) {
			this.wrt("<option value='"+i+"'")
		...
		this.wrt("</table>");
		this.wrt("<div style='text-align:center;padding:2px 0 4px 0'><input type='button' value='关闭' onclick='calendar.hideCalendar();' /></div>");
