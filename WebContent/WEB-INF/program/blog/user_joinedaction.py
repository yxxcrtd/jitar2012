...
class user_joinedaction(BaseAction, RequestMixiner, ResponseMixiner):
    ...
    def execute(self):
        ...
        fc = FileCache()
        content = fc.getUserFileCacheContent(self.userName, "user_joinedaction.html",14400)
        if content != "":
        ...
        content = templateProcessor.processTemplate(map, "/WEB-INF/user/default/user_joinedaction.ftl", "utf-8")
        
        fc.writeUserFileCacheContent(self.userName, "user_joinedaction.html",content)        
        response.getWriter().write(content)
        fc = None
