...
<head>
	...
	<title>用户个人资源分类管理</title>
	<link rel="stylesheet" type="text/css" href="../css/manage.css" />
	<script type="text/javascript">
	<!--
	function confirm_delete(name) {
		if (confirm("您确定要删除 " + name + " 分类吗？\r\n\r\n注意: 分类删除之后, 该分类所有资源将被设置为未分类的.")) {
		  return confirm('请再次确定您要删除该分类, 分类删除之后, 该分类所有资源将被设置为未分类的.');
		} else 
		  return false;
	}
	//-->
...
<table class="listTable" cellspacing="1">
	...
	<tbody>
		...
		<tr>
			...
			<td align="center">${category.itemNum}</td>
			<td>
...
<div class='funcButton'>
<form name='addCateForm' action='rescate.action' method='get'>
...
<form name='setOrderForm' action='category_Order.py' method='post' style='display:none'>
  ...
  <input type='hidden' name='type' value='user_res_${user.userId}' />
