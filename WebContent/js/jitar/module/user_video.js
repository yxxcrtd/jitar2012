...
(function() {
  ...
  var m = App._module_infos.user_video;
  ...
  function _load_profile(/*widget*/w) {
  	...
  	var url = CommonUtil.getCurrentRootUrl() + 'js/jitar/module/user_video.py?userId=' + user.id + '&count=' + count + '&tmp=' + (new Date()).getTime();
  ...
  m.load = function(widget) {
      var w = widget;
      logger.log('user_video module.load() is called widget = ' + w);
      w.setContent($L('加载中......'));
       if(w._btn_edit)
      {
  ...
  m.edit = function(widget) {
    ...
    if(t)
    {
      ...
      t.cells[1].innerHTML = "<div style='text-align:center;'>" +
              "显示视频数：<input id='ListCount_" + w.id + "' value='" + count + "' style='width:30%;' /><br/>" + 
  ...
  delete m.loading;
  m.loaded = true;
  logger.log('user_video module js is loaded');
  App.onModuleLoaded('user_video');
