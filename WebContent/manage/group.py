...
# 协作组管理.
class group (ActionExecutor, ShowGroupBase, PageCheckMixiner, ResponseMixiner):
  def __init__(self):
  ...
  # 列出我创建的协作组.
  def list(self):
    qry = __ComplexGroupQuery(""" g.groupId, g.groupName, g.groupTitle, g.groupIcon, g.createDate, g.parentId,
      g.groupState, g.userCount, g.groupTags, g.groupIntroduce """)
    qry.createUserId = self.loginUser.userId
    qry.groupState = None     # 列出所有.
    qry.parentId=None  # 列出所有.
    group_list = qry.query_map(-1)
    request.setAttribute("group_list", group_list)
    return "/WEB-INF/ftl/group/list.ftl"
    

  # 列出我加入的协作组.
  def list_join(self):
    qry = __ComplexGroupQuery(""" gm.groupRole, gm.status, gm.joinDate, g.groupId, g.groupName, g.groupTitle, g.parentId,
      g.groupIcon, g.createDate, g.createUserId, g.groupState, g.userCount, g.groupTags, g.groupIntroduce """)
    # 协作组正常、锁定、隐藏的可以进入.
  ...
  # 我发出的邀请.
  def my_invite(self):
    qry = __ComplexGroupQuery(""" gm.groupId, gm.userId, gm.joinDate, gm.status, g.parentId,
          g.groupTitle, g.groupTags, g.groupIntroduce, gmu.nickName, gmu.loginName,
    ...
    #设置为None 表示不限制组，查找全部，=0 只查主群组   >0查找子群组
    qry.parentId=None 
  ...
  # 我收到的邀请, 邀请我加入协作组的.
  def invite_me(self):
    qry = __ComplexGroupQuery(""" gm.groupId, gm.inviterId, gm.joinDate, gm.status, g.parentId,
          inv.userId, inv.nickName, inv.loginName, inv.userIcon, inv.blogName, 
          g.groupTitle, g.groupTags, g.groupIntroduce """)
    # 协作组正常、隐藏的可以邀请.
    qry.arrGroupState = [Group.GROUP_STATE_NORMAL, Group.GROUP_STATE_HIDED]
    # 成员状态为被邀请.
    qry.arrMemberState = [GroupMember.STATUS_INVITING]
  ...
  def my_joinreq(self):
    qry = __ComplexGroupQuery(""" gm.groupId, gm.inviterId, gm.joinDate, gm.status, g.parentId,
          g.groupIcon, g.groupTitle, g.groupTags, g.groupIntroduce,
  ...
  def req_me(self):
    ...
    # 查询这些组中的加入申请.
    qry = __ComplexGroupQuery(""" gm.id, gm.groupId, gm.userId, gm.joinDate, gm.status, g.parentId,
            g.groupTitle, g.groupTags, g.groupIntroduce, gmu.nickName, gmu.loginName, gmu.userIcon, gmu.blogName """)
  ...
  def __home_video_list(self):
    qry = GroupVideoQuery(""" v.videoId, v.title, v.createDate, v.href,v.flvThumbNailHref """)
    qry.groupId = self.group.groupId
    qry.shareMode = Resource.SHARE_MODE_GROUP
  ...
  def get_current_group(self):
    ...
    #print "self.group.parentId="+str(self.group.parentId)
