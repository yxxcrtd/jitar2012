  ...
  <dt><a target="_blank" href="${ContextPath}u/${senderUser.loginName!}"><img src="${SSOServerUrl +'upload/'+ senderUser.userIcon!""}" width="50" onerror="this.src='${ContextPath}images/default.gif'"/></a></dt>
    <dd>
      <div class="replyContent" id="replyContent${comment.id}">
          <div id="r${comment.id}">
                <h4><em class="emDate">${comment.createDate?string("yyyy-MM-dd HH:mm:ss")}</em><a target="_blank" href="${ContextPath}u/${senderUser.loginName!}">${senderUser.trueName!"未知"}</a></h4>
            ...
            <!--回复中的回复内容 Start-->
            <#if comment.replyList?? && comment.replyList?size &gt; 0 >
