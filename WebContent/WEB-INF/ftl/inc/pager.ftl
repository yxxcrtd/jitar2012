...
<span>
<#if pager??>
<input type="hidden" name="ppp" value="${pager.currentPage}"/>
...
<#if (pager.totalPages > 0)>转到页面:<input id='__pg' style='width:60px' value='${pager.currentPage}' /> <input type='button' value='查看' onclick="window.location='${pager.urlPattern}'.replace('{page}', document.getElementById('__pg').value).replace('{total}', '${pager.totalPages}');" /></#if>
</#if>
</span>
