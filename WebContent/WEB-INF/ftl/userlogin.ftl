...
<head>
  ...
  var isHide = true;
  function showImg(){
    if(isHide){document.getElementById("_userimg").src="${casServerUrl!}/authimg?"+Date.parse(new Date());}
  }
  //]]>
  </script>
</head>
...
<div class='main' style="width:180px;">  
  <form id='userlogin' method='post' action="${casServerUrl}/redirectlogin" style='padding-top:8px;' target="_top">
    <input type="hidden" id="targetService" name="service" value="${serverName}">
    <input type="hidden" name="ru" value="${siteserverurl}/login/">
    <input type="hidden" name="failpage" value="${siteserverurl}">
    <input type="hidden" name="callbackLogOutUrl" value="${callbackLogOutUrl}">
