...
<h2>推送文章审核</h2>
<#if content_list??>
<form action='admin_mashup_article_list.py' style='text-align:right;padding-bottom:6px'>
...
<option value='unitTitle'<#if f?? && f == 'unitTitle'> selected='selected'</#if>>作者机构</option>
...
<form id='oForm' method='POST' action='${SiteUrl}mashup/admin_mashup_article_list.py'>
...
<td><a href='${a.href}go.py?articleId=${a.orginId}' target='_blank'>${a.title!?html}</a></td>
<td>${a.author!?html}</td>
...
<input type='button' class='button' value='设为待删除 ' onclick='doPost("todelete")' />
-->
</form>
<#else>
没有需要推送的文章。
