...
<body>
<h2>自定义分类文章管理</h2>
<form method='GET' action='subject_contentspace_article_list.py'>
<input name='id' value='${subject.subjectId}' type='hidden'/>
...
<th>
<select onchange='window.location.href="subject_contentspace_article_list.py?id=${subject.subjectId}&contentSpaceId=" + this.options[this.selectedIndex].value'>
<option value=''>全部文章分类</option>
<#if category_tree??>
<#list category_tree.all as c>
<option value='${c.categoryId}'<#if contentSpaceId==c.categoryId> selected='selected'</#if>>${c.treeFlag2} ${c.name?html}</option>
...
<td>
<a href='preview_contentspace_article.py?id=${subject.subjectId}&contentSpaceArticleId=${a.contentSpaceArticleId}' target='_blank'>${a.title!?html}</a></td>
<td>${a.spaceName}</td>
...
<td><nobr><a href='subject_content_space_article_add.py?id=${subject.subjectId}&contentSpaceArticleId=${a.contentSpaceArticleId}'>修改</a></nobr></td>
</tr>
</#list>
</table>
</#if>
...
<div style='padding:6px'>
  ...
  <input class='button' type='button' value='添加文章' onclick='window.location.href="subject_content_space_article_add.py?id=${subject.subjectId}"' />
