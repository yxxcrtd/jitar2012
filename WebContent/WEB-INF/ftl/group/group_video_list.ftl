...
<form name='theForm' id='theForm' action='groupVideo.action' method='post'>
  <input type='hidden' name='cmd' value='list' />
  <input type='hidden' name='groupId' value='${group.groupId}' />
  ...
  <#list video_list as video>
    <tr>
      ...
      <td>
        ...
        <#if video.isGroupBest><img src='images/ico_best.gif' border='0' align='absmiddle' /></#if>
      </td>
      <td>
        <#if video.groupCategory??>
          ${video.groupCategory.name!?html}
        </#if>
      </td>
      <td>
        <#assign user = Util.userById(video.userId)>
        <a href='${SiteUrl}go.action?loginName=${user.loginName}' target='_blank'>${user.nickName!?html}</a> (${video.createDate?string('yyyy-MM-dd')})
      </td>
      <td align='right'>${video.downloadCount}/${video.commentCount}</td>
    </tr>
  </#list>
  </tbody>
</table>
...
<#if can_pub >
  <input type='button' class='button' value='  上传视频  ' onclick='document.uploadForm.submit();' />
</#if>
...
<form name='uploadForm' action='video.action' method='get'>
...
function showDialog()
{
    ...
    htmlContent+="<option value=\"\">群组视频分类</option>";
    htmlContent+=" <option value=\"\">不设置群组视频分类</option>";
    <#list res_cate.all as category>
    htmlContent+=" <option value=\"${category.id}\">${category.treeFlag2 + category.name!?html}</option>";
    </#list>
    htmlContent+=" </select>";
    htmlContent+="  </div>";
    msg("视频移动到群组分类",2,"280",htmlContent,"move_r()")
}
...
function select_all() {
  var ids = document.getElementsByName('videoId');
  if (ids == null) return;
  var has_set = false;
...
function show_move_r() {
  var div = document.getElementById('move_video_to');
...
function unref_r() {
  ...
  if (confirm('您是否确定要删除选定的视频? (视频只是取消了到群组的引用, 未实际删除)') == false) return;
  var f = document.forms.theForm;
  f.cmd.value = 'unref';
  f.submit();
}
...
function group_changed() {
  ...
  frame.document.location.href = 'video.action?cmd=dest_cate&groupId=' + groupId;
}
