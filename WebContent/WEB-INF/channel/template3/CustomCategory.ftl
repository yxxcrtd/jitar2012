...
<li>
<a href="${SiteUrl}showArticle.action?articleId=${article.articleId}" target='_blank' title='${article.title!?html}'>${article.createDate?string("yyyy年MM月dd日")}:${Util.getCountedWords(article.title!?html,24)}</a></li>
</#list>
</ul>
</#if>
