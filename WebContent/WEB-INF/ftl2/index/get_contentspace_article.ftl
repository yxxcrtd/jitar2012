<#if showType == 1>
  <#if article_list??>
	...
	<#list article_list as article>
	 <li><em class="emDate">${article.createDate?string('MM-dd')}</em>
	     <a href='showContent.action?articleId=${article.contentSpaceArticleId}' target='_blank' <#if textLength &lt; article.title?length>  title='${article.title!?html}'</#if>>${Util.getCountedWords(article.title!?html,textLength)}</a>
	 </li>
	</#list>
	</ul>
  </#if>
<#elseif showType == 2 >
	<#if picture_article_list?? >
	<#assign pics = ''>
	<#assign links = ''>
	<#assign texts = ''>
	<#list picture_article_list as article >
	 <#if article_has_next>
	  <#assign pics = pics + Util.url(article.pictureUrl!"") + '|' >
	  <#assign links = links + SiteUrl + 'showContent.action?articleId=${article.contentSpaceArticleId}|' >
