...
<ul class='news_new_item_ul'>
  <#list article_list as sa>		  
	  <li><span>${sa.createDate?string('yyyy-MM-dd')}</span>
		<a href='${SiteUrl}showContent.action?articleId=${sa.contentSpaceArticleId}' target='_blank'>${sa.title!?html}</a> (${sa.viewCount})
	  </li>
	</#list>
</ul>
</#if>
  
<div style='text-align:center'><#include '/WEB-INF/ftl/inc/pager.ftl' ></div>
