...
public class Uploader {
	...
	public Uploader(HttpServletRequest request,String loginName) {
		this.request = request;

		if(null == loginName || loginName.length() == 0){
			//重新获得一次 
			loginName = this.getParameter("loginName");
		}
		if(null == loginName || loginName.length() == 0){
			//loginName还是没有，从Session里获得 
			cn.edustar.jitar.pojos.User user = WebUtil.getLoginUser(request.getSession());
			if(null != user){
				loginName = user.getLoginName();
			}
		}
		
		this.savePath = "user/" + loginName;
	...
	private String getPhysicalPath(String path) {
		...
		*/
		...
		if (null != fileConfigPath && !"".equals(fileConfigPath)) {
			returnPath = fileConfigPath + File.separator + path;
		}else{
	    	//ApplicationContext ac = WebApplicationContextUtils.getWebApplicationContext(request.getServletContext());   
	    	//cn.edustar.jitar.service.StoreManager sto_mgr = (cn.edustar.jitar.service.StoreManager) ac.getBean("storeManager");
			//FileStorage store = sto_mgr.getUserFileStorage(user);
			//File user_root = store.getRootFolder();
			returnPath =  this.request.getSession().getServletContext().getRealPath("/") + path;
		}
		return returnPath;
	}
