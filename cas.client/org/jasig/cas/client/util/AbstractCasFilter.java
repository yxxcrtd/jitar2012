...
//        CommonUtils.assertTrue(CommonUtils.isBlank(this.serverName) || CommonUtils.isBlank(this.service), "serverName and service cannot both be set.  You MUST ONLY set one.");
    ...
    protected final String constructServiceUrl(final HttpServletRequest request, final HttpServletResponse response) {
    	if (CommonUtils.isEmpty(this.serverName) && CommonUtils.isEmpty(this.service)) {
    		this.serverName = CommonUtils.getSiteServer(request);
    	}
    	log.info("获取到的serverName是：" + this.serverName);
    	log.info("service是：" + this.service);
