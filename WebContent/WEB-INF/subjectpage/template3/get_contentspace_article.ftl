	...
	<ul class='item_ul'>
		<#list article_list as article>
			<li>
				...
				<a href='${SubjectRootUrl}py/showContent.py?articleId=${article.contentSpaceArticleId}' target='_blank' title='${article.title!?html}'>${article.title!?html}</a> [${article.viewCount}]
			</li>
		</#list>
		</ul>
  </#if>
<#elseif showType == 2 >
	...
	<#if article_index < 6 && article.pictureUrl??>
	 <#if article_has_next>
	  ...
	  <#assign links = links + SubjectRootUrl + 'py/showContent.py?articleId=${article.contentSpaceArticleId}|' >
