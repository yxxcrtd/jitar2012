<#if showType == 1>
  <#if article_list??>
	...
	<#list article_list as article>
	 <li><span>${article.createDate?string('yyyy-MM-dd')}</span><a href='showContent.action?articleId=${article.contentSpaceArticleId}' target='_blank' title='${article.title!?html}'>${article.title!?html}</a> [${article.viewCount}]</li>
	</#list>
	</ul>
  </#if>
<#elseif showType == 2 >
