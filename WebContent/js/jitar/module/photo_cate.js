...
(function() {
  ...
  var m = App._module_infos.photo_cate; 
  // 如果已经加载过了，则不要重复加载
  if (m.loaded) return;
  ...
  function _load_profile(/*widget*/w) {
    //var url = CommonUtil.getCurrentRootUrl() + 'js/jitar/module/photo_category.py?loginName=' + user.name + '&tmp=' + Date.parse(new Date());
	  var url = CommonUtil.getCurrentRootUrl() + 'u/' + user.name + '/py/photo_category.py?tmp=' + (new Date()).getTime();
