...
(function() {
  var m = App._module_infos.category_video;
  // 如果已经加载过了，则不要重复加载.
  if (m.loaded) return;
  // 加载成功，显示内容。
  ...
  function _load_category_article(widget) {
	var url = CommonUtil.getCurrentRootUrl() + 'js/jitar/module/category_video.py?loginName=' + user.name + '&userCateId=' + widget.data.categoryId + '&count=' + widget.data.count + '&title=' + encodeURIComponent(escape(widget.data.title)) + '&tmp=' + (new Date()).getTime();
  ...
  m.edit = function(widget) {
  	...
  	if(t)
  	{
  		...
  		var url = CommonUtil.getCurrentRootUrl() + 'u/' + user.name + '/py/get_user_videocategory.py?wid='+ w.id +'&oldCateId=' + oldCateId + '&count='+ count +'&title='+ encodeURIComponent(escape(title)) +'&tmp=' + (new Date()).getTime();
  		new Ajax.Request(url, { 
	          method: 'get',
	          onSuccess:function(xhr){
  ...
  delete m.loading;
  m.loaded = true;
  App.onModuleLoaded('category_video');
}) ();
