...
<head>
  ...
  function addModule()
  {
  	window.location.href='add_custorm_module.py?id=${subject.subjectId}';
  }
  function addSysModule()
  {
  	window.location.href='add_sys_module.py?id=${subject.subjectId}';
  }
  function addContentSpaceModule()
  {
   window.location.href='add_contentspace_module.py?id=${subject.subjectId}';
  }
...
<tr>
<td style='width:17px'><input type='checkbox' name='guid' value='${webpart.subjectWebpartId}' /></td>
...
<td>
<#if webpart.partType == 0>
<a href='add_custorm_module.py?id=${subject.subjectId}&amp;webpartId=${webpart.subjectWebpartId}'>修改</a>
<#elseif webpart.partType == 1>
<a href='add_sys_module.py?id=${subject.subjectId}&amp;webpartId=${webpart.subjectWebpartId}'>修改</a>
<#elseif webpart.partType == 2>
<a href='add_contentspace_module.py?id=${subject.subjectId}&amp;webpartId=${webpart.subjectWebpartId}'>修改</a>
<#else>
未定义的类型
</#if>
</td>
...
<div style='padding:6px'>	
	<input class='button' type='button' value='添加自定义内容模块' onclick='addModule()' />
