...
/**
    ...
    public static String constructServiceUrl(final HttpServletRequest request,
        ...
        if (CommonUtils.isNotBlank(request.getQueryString())) {
            ...
            if (location == 0) {
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug("serviceUrl generated 1: " + returnValue);
                }
                return returnValue;
            }
        ...
        final String returnValue = encode ? response.encodeURL(buffer.toString()) : buffer.toString();
        if (LOG.isDebugEnabled()) {
            LOG.debug("serviceUrl generated 2: " + returnValue);
        }
        return returnValue;
    }
    ...
    public static String getSiteServer(HttpServletRequest request) {
    	String root = "";
    	//此处有可能丢失端口号？把下面的  getServerPort 改成 getLocalPort测试 了
    	//mxh
    	if (80 == request.getLocalPort()) {
    		root = request.getScheme() + "://" + request.getServerName();
    	} else {
    		root = request.getScheme() + "://" + request.getServerName() + ":" + request.getLocalPort();
    	}
	...
	/**
	 * 得到默认的统一用户的登录地址
	 * 见web.xml中的 CAS-Authentication-Filter 参数 casServerLoginUrl
	 * 如果该参数没有设置的话，自动获得 这个
	 * @return
	 */
	public static String getCasServerLoginUrl(HttpServletRequest request){
		String url=getSiteServer(request)+"/"+JitarConst.USERMGR_SITENAME+"/login";
		return url;
	}
	
	/**
	 * 得到默认的统一用户的地址
	 * 见web.xml中的CASValidationFilter 参数 casServerUrlPrefix
	 * 如果该参数没有设置的话，自动获得 这个
	 * @return
	 */
	public static String getCasServerUrlPrefix(HttpServletRequest request){
		String url=getSiteServer(request)+"/"+JitarConst.USERMGR_SITENAME+"";
		return url;
	}
	

	public static String getHttpURLContent(String url)throws ClientProtocolException, IOException { 
		...
		HttpClient httpclient = new DefaultHttpClient(); 
         ...
         String result = ResponseHander.handleResponse(response);
