...
<head>
<meta charset="utf-8">
<title><#include '/WEB-INF/ftl2/common/site_title.ftl' /> - 课题资源</title>
...
<div class="secRightW mt3 border">
    <h3 class="h3Head textIn" style="width:1000px">课题成果-资源</h3>
    <!--循环 Start-->
        ...
        <div class="listCont">
          ...
          <#if resource_list??>
            <#list resource_list as resource>
              <#if resource_index % 2 = 1>
               <li class="liBg ${Util.iconCss(resource.href!)}">
              <#else> 
              <li class="${Util.iconCss(resource.href!)}">
              </#if>  
                    <span style="width:50%"><a href='${ContextPath}showResource.action?resourceId=${resource.resourceId}' target='_blank' title="${resource.title!?html}">${Util.drawoffHTML(resource.title, 23)}</a></span>
                    <span style="width:30%">${resource.groupTitle}</span>
                    <span style="width:20%">${resource.createDate?string('yyyy-MM-dd')}</span>
                </li>
                </#list>
           </#if>
          </ul>   
