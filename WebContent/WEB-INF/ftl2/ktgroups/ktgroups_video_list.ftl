...
<head>
<meta charset="utf-8">
<title><#include '/WEB-INF/ftl2/common/site_title.ftl' /> - 课题视频</title>
...
<div class="secRightW mt3 border">
    <h3 class="h3Head textIn" style="width:1000px">课题成果-视频</h3>
    <!--循环 Start-->
        ...
        <div class="listCont">
          ...
          <#if video_list??>
            <#list video_list as video>
              <#if video_index % 2 = 1>
               <li class="liBg" style="height:133px">
              <#else> 
              <li style="height:133px">
              </#if>  
                    <span style="width:50%">
                        <a href='${ContextPath}manage/video.action?cmd=show&videoId=${video.videoId}' title="${video.title!}"><img border=0 src="${video.flvThumbNailHref!}"/></a>
                        <a href='${ContextPath}manage/video.action?cmd=show&videoId=${video.videoId}' target='_blank' title="${video.title!}">${Util.drawoffHTML(video.title, 23)}</a>
                    </span>
                    <span style="width:30%">${video.groupTitle}</span>
                    <span style="width:20%">${video.createDate?string('yyyy-MM-dd')}</span>
                </li>
                </#list>
           </#if>     
          </ul>   
