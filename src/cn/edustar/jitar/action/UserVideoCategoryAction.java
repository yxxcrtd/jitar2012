 ...
 *
 */
public class UserVideoCategoryAction extends ManageBaseAction {
	
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 3074665366559610950L;
	...
	private String getCategoryItemType() {
		return CommonUtil.toUserVideoCategoryItemType(getLoginUser().getUserId());
	}
	...
	private String delete() {
		...
		// 更新使用此分类的视频, 将其 userCateId 都设置为 NULL.
		video_svc.batchClearVideoUserCategory(category.getCategoryId());
		// 执行删除.
		cate_helper.getCategoryService().deleteCategory(category);
		
		addActionMessage("分类 " + category.getName() + " 成功删除");
		FileCache fc = new FileCache();
		fc.deleteUserVideoCate(getLoginUser().getLoginName());
		fc = null;
		return SUCCESS;
	}
