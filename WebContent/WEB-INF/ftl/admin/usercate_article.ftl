...
<body>
<h2>自定义分类文章管理</h2>
<form method='GET' action='usercate_article.py'>
...
<#if category_tree??>
  <#list category_tree.all as c>
     <option value='${c.categoryId}' ${(c.categoryId == contentSpaceId)?string('selected', '')}>${c.treeFlag2} ${c.name?html}</option>
  </#list>
</#if>
</select>
...
<th>
<select onchange='window.location.href="usercate_article.py?contentSpaceId=" + this.options[this.selectedIndex].value'>
<option value=''>全部文章分类</option>
<#if category_tree??>
  <#list category_tree.all as c>
        <option value='${c.categoryId}' ${(c.categoryId == contentSpaceId)?string('selected', '')}>${c.treeFlag2} ${c.name?html}</option>
...
<td>
<#if a.pictureUrl?? && a.pictureUrl != "" ><span style="color:#f00">[图]</span></#if>
...
<td><nobr><a href='content_space_article_edit.py?from=article&id=${a.contentSpaceId}&contentSpaceArticleId=${a.contentSpaceArticleId}'>修改</a></nobr></td>
</tr>
</#list>
</table>
</#if>
...
<div style='padding:6px'>
  ...
  <input class='button' type='button' value='添加文章' onclick='window.location.href="content_space_article_add.py"' />
  <input class='button' type='button' value='彻底删除' onclick="if(checkSelectedItem() && window.confirm('您真的要删除这些文章吗？')){doPost('delete');}" />
  ...
  <input class='button' type='button' value='执行移动分类' onclick="if(checkSelectedItem()) {doPost('move_cate');}" />
</div>
