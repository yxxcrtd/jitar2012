...
<a href="${SiteUrl}go.action?id=${subject.subjectId}" target='_blank'>学科网站首页</a> | 
<a href="${SiteUrl}subject/manage/admin_index.py?id=${subject.subjectId}" target='_top'>学科管理首页</a> | 
<a href="${SiteUrl}help/xkhelpindex.html" target='main'>学科管理使用帮助</a> | 
<a href="${SiteUrl}logout.jsp" target='_top'>退出登录</a>
</div>
<div style='float:left'><span style='color:#F00;padding-right:50px;'>学科名称：${subject.subjectName}</span> 欢迎您：${loginUser.trueName!?html}</div>
