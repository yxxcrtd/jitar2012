...
class user_rcate(BaseAction):
    def execute(self):
        ...
        fc = FileCache()
        # 14400 为10 天
        content = fc.getUserFileCacheContent(self.userName, "user_res_cate.html",14400)
        if content != "":
            response.getWriter().write(content)
            fc = None
            return
        
        itemType = CommonUtil.toUserResourceCategoryItemType(user.getUserId())        
        ...
        content = templateProcessor.processTemplate(map, "/WEB-INF/user/default/user_res_cate.ftl", "utf-8")
        
        fc.writeUserFileCacheContent(self.userName, "user_res_cate.html",content)        
        response.getWriter().write(content)
        fc = None
